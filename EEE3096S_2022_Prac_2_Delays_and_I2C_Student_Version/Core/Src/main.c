...
uint16_t bitpattern = 0b00001100000000;
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
typedef struct {
	uint8_t seconds;
...
int main(void){
  ...
  while (1)
  {
	...
	//push button to turn led off for a specified delay
	...
	//TASK 6
	getTime();
	sprintf(buffer, "%d:%d:%d  %d \r\n",time.hour, time.minutes, time.seconds, epochFromTime(time));
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	//YOUR CODE HERE
	uint8_t FIRST_REG = 0x00; //the first register is the 00h register showing seconds
	uint8_t REG_SIZE = 1;

	//defining what to write to the registers
...
void getTime (void)
{
	...
	//YOUR CODE HERE
	//assigning read values to the TIME structure
...
int epochFromTime(TIME time){
	...
	int minutes = hours*60 + time.hour;
